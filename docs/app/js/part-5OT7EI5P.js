...
async function checkCredentialsFileSafety(path2, onWarn = (m) => console.warn(`anthropic-sdk: ${m}`)) {
  if (typeof process === "undefined" || process.platform === "win32")
    return;
  const { fs } = await import("./part-W64CD333.js");
  let resolved = path2;
  let st;
  try {
...
async function writeCredentialsFileAtomic(targetPath, data) {
  const { fs, path: path2 } = await import("./part-W64CD333.js");
...
var getCredentialsPath = async (config, profile) => {
  ...
  validateProfileName(profileName);
  const { path: path2 } = await import("./part-W64CD333.js");
...
async function _EnvironmentWorker_handleItem2(work, environmentKey, externalSignal) {
  ...
  const agentToolset = await import("./part-R7XNFKS3.js");
  let leaseTtlMs;
  let runner;
...
var Workspaces3 = class extends APIResource {
  constructor() {
    super(...arguments);
...
var BaseAnthropic = class {
  ...
  async makeRequest(optionsInput, retriesRemaining, retryOfRequestLogID) {
    ...
    if (!response.ok) {
      ...
      const errMessage = errJSON ? void 0 : errText2;
